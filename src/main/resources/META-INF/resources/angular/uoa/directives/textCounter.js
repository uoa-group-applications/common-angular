 ...
 *
 * <textarea uoa-text-counter min-length="\d+" max-length="\d+"></textarea>
 *
 * the counter label will be put on the closest .row or .row-role class, so this
 * works particularly well in combination with the uoa-form-field directive.
 *
 * depending on its state the counter label has the "good" and "bad" class, make sure
 * these are styled somewhere.
 */
UOA.common.directive("uoaTextCounter", ['utils', function() {
