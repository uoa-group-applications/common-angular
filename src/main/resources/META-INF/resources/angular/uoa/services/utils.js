 ...
 */
window.template = function(base, templateName, app) {
...
window.getFixedBreadcrumbs = function() {
    if (angular.isDefined(UOA.permanentBreadcrumb)) {
        return UOA.permanentBreadcrumb;
    }
    else {
        return [];
    }
 ...
 */
angular.module("uoa-common").factory("utils", ['$rootScope', '$compile', function($rootScope, $compile) {
    ...
    window._t = $rootScope._t = function(label) {
        return angular.isDefined(UOA.i18n[label]) ? UOA.i18n[label] : label;
    };
